...
import {
    ...
    Paper, Typography, TextField, Dialog, DialogActions, DialogContent, DialogTitle,
    IconButton, MenuItem, Select, FormControl, InputLabel, Alert, Box,
} from '@mui/material';
...
const ADMIN_MOVIES_URL = `${BASE_URL}/admin/movies`;
...
export default function MovieManagement() {
    ...
    const fetchMovies = async () => {
        try {
            const data = await apiRequest(API_URLS.ADMIN.movie.list_movie, { method: 'GET' });
            setMovies(data);
        } catch (err: any) {
    ...
    const handleEdit = async (movie: Movie) => {
        try {
            const data = await apiRequest(API_URLS.ADMIN.movie.detail(movie.id), { method: 'GET' });
    ...
    const handleDelete = async (id: number) => {
        ...
        try {
            await apiRequest(API_URLS.ADMIN.movie.delete(id), { method: 'DELETE' });
            fetchMovies();
        } catch (err: any) {
